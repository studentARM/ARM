    ...
    , ui(new Ui::MainWindowLeader)
{
    ...
    if (!db.open()) {
        ...
        if (!query.exec(createLabsTable)) {
            qDebug() << "Ошибка создания таблицы labs: " << query.lastError().text();
        }
        ...
        if (!query.exec(createTestsTable)) {
            qDebug() << "Ошибка создания таблицы tests: " << query.lastError().text();
        }
    }
