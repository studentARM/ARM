    ...
    , armWindow(armWindow)  // Получаем указатель на ARM окно
{
    ...
    if (!emailExists) {
        ...
        if (!alterQuery.exec("ALTER TABLE users ADD COLUMN email TEXT")) {
            qDebug() << "Ошибка добавления столбца email:" << alterQuery.lastError().text();
        } else {
            qDebug() << "Столбец email успешно добавлен.";
        }
    }
...
void LoginWindow::on_btnLogin_clicked()
{
    ...
    if (query.next()) {
        ...
        if (storedEmail != email) {
            ...
            if (!updateQuery.exec()) {
                qDebug() << "Ошибка обновления email:" << updateQuery.lastError();
            }
        }
